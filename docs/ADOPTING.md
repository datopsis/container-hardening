...
| The list of controls to inspect | [`INSPECTION-LIST.md`](../examples/reference-web-server/INSPECTION-LIST.md), generated by [`scripts/build-inspection-list.py`](../scripts/build-inspection-list.py) | The capabilities in the profile, which decide the list |
| Diagrams | [`scripts/build-package-diagrams.py`](../examples/reference-web-server/scripts/build-package-diagrams.py) | The image's own context, flows, boundaries, sequences, lifecycle, deployment, network, and identity |
| A cyber package | [`package/README.md`](../examples/reference-web-server/package/README.md) | All of it: this is the part a person writes |
...
## 7b. Draw the image, and write its cyber package

The standard lists every control. Which ones this image must inspect follows
from what it does, and where each is applied is something only a person can
say. Both live in the image's **cyber package**, defined in
[The cyber package](CYBER-PACKAGE.md).

**Declare what the image does.** In the profile, from the vocabulary in the
[inspection map](../artifacts/inspection-map.json):

```json
"capabilities": ["serves-http", "terminates-tls"]
```

Each capability brings controls onto the list; declaring
`terminates-mutual-tls` or `authenticates-clients` brings identification and
access enforcement with it. Generate the list, which covers every control in
the baseline in three groups, so a reviewer can walk all of them:

```sh
python ../container-hardening/scripts/build-inspection-list.py \
    --profile hardening-profile.json --component oscal/component-definition.json \
    --decisions decisions.json --image my-image --out INSPECTION-LIST.md
```

**Draw it.** Copy the reference image's
[`build-package-diagrams.py`](../examples/reference-web-server/scripts/build-package-diagrams.py),
which uses [`scripts/svg.py`](../scripts/svg.py) from this repository, and
describe the image in it: its context, its data flows and **trust
boundaries**, a sequence for each path that matters, its lifecycle, its
deployment, its network, and its identity and privilege. Generating them keeps
them in step with the image; run the generator with `--check` in CI, as with
every other generated page.

A boundary is the part a reader cannot infer. Say where something stops being
trusted: what the platform admits and imposes, what the container carries,
what is mounted, and what never crosses back out.

**Write the package.** Eight sections, listed in
[The cyber package](CYBER-PACKAGE.md#what-a-package-contains): what the image
is, its architecture, its data flow and boundaries, where the controls are
applied, the controls themselves, the evidence, the deviations and what is
open, and what the package does not claim. Then check its form:
...
That check reads the form only: a missing section, an undiscussed capability, an
unreferenced diagram, a broken link, a flagged control the package ignores, a
placeholder. Whether the architecture is described correctly is review, and
always will be.
