...
class Suite:
    ...
    def record(self, criterion: str, check_id: str, check: str, passed: bool | None, detail: str = "") -> None:
        ...
        self.results.append({"id": check_id, "criterion": criterion, "check": check, "passed": outcome, "detail": detail})
...
def static_checks(suite: Suite, revision: str | None) -> None:
    ...
    suite.record("IMG-11", "smoke.user-numeric-nonroot-group-0", "configured user is numeric, non-zero, with group 0", uid.isdigit() and int(uid) != 0 and gid == "0", user)
    ...
    suite.record("IMG-20", "smoke.healthcheck-exec-form", "HEALTHCHECK is declared in exec form", bool(health) and health[0] == "CMD", str(health))
    suite.record("IMG-20", "smoke.stopsignal-declared", "STOPSIGNAL is declared", bool(config["Config"].get("StopSignal")), str(config["Config"].get("StopSignal")))
    ports = [int(p.split("/")[0]) for p in (config["Config"].get("ExposedPorts") or {})]
    suite.record("IMG-14", "smoke.exposed-ports-unprivileged", "every exposed port is 1024 or above", bool(ports) and min(ports) >= 1024, str(ports))
    ...
    suite.record("IMG-23", "smoke.labels-present", "identifying labels are present", not missing, ", ".join(missing))
    if revision:
        suite.record("IMG-23", "smoke.revision-label-matches", "revision label equals the commit", labels.get("org.opencontainers.image.revision") == revision)
    ...
    suite.record("IMG-06", "smoke.no-package-manager", "no package manager or repository configuration", not any(p for p in present if "rpm" in p or "dnf" in p or "yum" in p), ", ".join(present))
    keys = [n for n in members if re.search(r"(RPM-GPG-KEY|\.repo$)", n)]
    suite.record("IMG-06", "smoke.no-repository-files", "no repository files or signing keys", not keys, ", ".join(keys[:5]))
    suite.record("IMG-07", "smoke.no-retrieval-tool-or-compiler", "no retrieval tool or compiler", not any(p in members for p in ABSENT if p.split("/")[-1] in ("curl", "wget", "gcc", "cc", "make")))
    suite.record("IMG-27", "smoke.no-remote-administration", "no remote administration daemon", not any(p in members for p in ABSENT if p.split("/")[-1] in ("sshd", "ssh", "telnetd")))
    suite.record("IMG-11", "smoke.no-su-or-sudo", "no su or sudo", not any(p in members for p in ("usr/bin/su", "usr/bin/sudo")))
    privileged = [n for n, m in members.items() if m.isfile() and m.mode & 0o6000]
    suite.record("IMG-09", "smoke.no-setuid-setgid", "no setuid or setgid file", not privileged, ", ".join(privileged[:5]))
    writable = [n for n, m in members.items() if not m.issym() and m.mode & 0o002 and n != "tmp"]
    suite.record("IMG-09", "smoke.no-world-writable", "nothing world-writable outside /tmp", not writable, ", ".join(writable[:5]))
    ...
    suite.record("IMG-10", "smoke.software-root-owned", "software and configuration root-owned, not group- or world-writable", bool(protected) and not loose, ", ".join(loose[:5]))
    manifest = members.get("usr/share/reference-web-server/rpm-manifest.tsv")
    if manifest is None:
        suite.record("IMG-08", "smoke.inventory-present", "package inventory present", False)
    else:
    ...
    suite.record("IMG-17", "smoke.no-trust-anchors-added", "the image adds no trust anchors", not bundles, ", ".join(bundles[:5]))
    log_files = [n for n, m in members.items() if n.startswith("var/log/") and m.isfile()]
    error_log = members.get("var/log/nginx/error.log")
    suite.record("IMG-19", "smoke.no-log-files", "no log file in the image; the error log links to stderr",
    ...
    suite.record("IMG-07", "smoke.dynamic-modules-declared", "no dynamic module beyond those declared", sorted(modules) == sorted(FEATURES["dynamic_modules"]), ", ".join(modules))


def runtime_checks(suite: Suite) -> None:
    name = suite.start()
    code = suite.http(name, "http://127.0.0.1:8080/")
    suite.record("IMG-15", "smoke.serves-read-only-root", "serves under a read-only root with only the declared tmpfs", code == 200, str(code))

    processes = proc_status(suite, name)
    root = [p for p in processes if p["Uid"].split()[0] == "0"]
    suite.record("IMG-11", "smoke.no-root-process", "no process runs as UID 0", bool(processes) and not root, str(root))
    caps = [p for p in processes if int(p["CapEff"], 16) != 0]
    suite.record("IMG-13", "smoke.no-effective-capabilities", "every process has an empty effective capability set", bool(processes) and not caps, str(caps))
    nnp = [p for p in processes if p.get("NoNewPrivs") != "1"]
    suite.record("IMG-13", "smoke.no-new-privileges", "every process has no_new_privs", bool(processes) and not nnp, str(nnp))
    seccomp = [p for p in processes if p.get("Seccomp") != "2"]
    suite.record("IMG-13", "smoke.seccomp-filtered", "every process runs under a seccomp filter", bool(processes) and not seccomp, str(seccomp))
    names = {p["Name"] for p in processes}
    suite.record("IMG-30", "smoke.processes-declared", "running processes are the declared ones", names <= set(BEHAVIOUR["processes"]), str(sorted(names)))

    ports = listening_ports(suite, name)
    declared = {listener["port"] for listener in BEHAVIOUR["listeners"]}
    suite.record("IMG-14", "smoke.listeners-unprivileged", "every listening socket is 1024 or above", bool(ports) and min(ports) >= 1024, str(sorted(ports)))
    suite.record("IMG-30", "smoke.listeners-declared", "listening sockets are declared", ports <= declared, str(sorted(ports)))

    write = suite.exec(name, "touch", "/usr/share/nginx/html/probe")
    suite.record("IMG-15", "smoke.root-write-refused", "a write to the root filesystem fails", write.returncode != 0, write.stderr.strip())
    config = suite.exec(name, "test", "-w", "/etc/nginx/nginx.conf")
    suite.record("IMG-10", "smoke.configuration-not-writable", "the runtime user cannot write configuration", config.returncode != 0)
    ...
    suite.record("IMG-07", "smoke.features-declared", "built features equal the declaration", flags == FEATURES["configure_flags"],
                 str(sorted(set(flags) ^ set(FEATURES["configure_flags"]))))

    logs = suite.podman("logs", name).stdout + suite.podman("logs", name).stderr
    suite.record("IMG-19", "smoke.requests-logged-to-stdout", "requests are logged to standard output", '"GET / HTTP' in logs)

    started = time.monotonic()
    stop = suite.podman("stop", "--time", "10", name)
    elapsed = time.monotonic() - started
    status = suite.podman("inspect", name, "--format", "{{.State.ExitCode}}").stdout.strip()
    suite.record("IMG-20", "smoke.stops-cleanly", "stops cleanly within the timeout", stop.returncode == 0 and status == "0" and elapsed < 10,
...
def arbitrary_uid(suite: Suite) -> None:
    ...
    try:
        ...
        suite.record("IMG-12", "smoke.arbitrary-uid", "serves under an arbitrary UID in group 0", False, error.stderr.strip()[-200:])
        return
    code = suite.http(name, "http://127.0.0.1:8080/")
    suite.record("IMG-12", "smoke.arbitrary-uid", "serves under an arbitrary UID in group 0", code == 200, str(code))
...
def user_namespace(suite: Suite) -> None:
    ...
    if probe.returncode != 0:
        suite.record("IMG-32", "smoke.user-namespace-available", "runs in a user namespace", None, "user namespaces unavailable here: " + probe.stderr.strip()[:120])
        return
    first = probe.stdout.split()
    suite.record("IMG-32", "smoke.user-namespace-remaps-root", "container UID 0 is not host UID 0", len(first) >= 2 and first[1] != "0", probe.stdout.strip())
    name = suite.start("--userns=auto")
    code = suite.http(name, "http://127.0.0.1:8080/")
    suite.record("IMG-32", "smoke.serves-in-user-namespace", "serves in a user namespace", code == 200, str(code))


def fails_closed(suite: Suite) -> None:
    missing = suite.podman("run", "--rm", *RESTRICTED, suite.image)
    output = missing.stdout + missing.stderr
    suite.record("IMG-15", "smoke.missing-tmpfs-fails-closed", "without the declared tmpfs it exits non-zero naming the path",
    ...
    with tempfile.TemporaryDirectory() as scratch:
        ...
        suite.record("IMG-18", "smoke.invalid-configuration-fails-closed", "invalid configuration exits non-zero naming the file and line",
...
def tls(suite: Suite) -> None:
    with tempfile.TemporaryDirectory() as scratch:
        ...
        suite.record("IMG-16", "smoke.tls-from-mounted-key", "serves TLS from read-only mounted key material", code == 200, str(code))
        writable = suite.exec(name, "test", "-w", "/etc/nginx/tls/server.key")
        suite.record("IMG-16", "smoke.key-not-writable", "the key is not writable by the runtime identity", writable.returncode != 0)
        leaks = []
        ...
        if secret in logs.stdout + logs.stderr:
            leaks.append("logs")
        suite.record("IMG-16", "smoke.key-not-disclosed", "no key material in arguments, environment, or logs", not leaks, ", ".join(leaks))
        ports = listening_ports(suite, name)
        declared = {listener["port"] for listener in BEHAVIOUR["listeners"]}
        suite.record("IMG-30", "smoke.tls-listeners-declared", "with TLS mounted, listening sockets are still declared", ports <= declared, str(sorted(ports)))
...
def main() -> int:
    ...
    evidence.write(args.evidence, evidence.image_subject(args.image), suite.results, podman=version)
