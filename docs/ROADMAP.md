...
## Where this stands, 20 September 2026

Work paused here to move to another project. Everything below the next two
sections is the remaining work; nothing is half-finished in the tree, and
`main` is green.

**What is in place.** The standard is 35 required criteria and four targets,
each stating its scope and whether a person's review may evidence it. The
control baseline derives 379 controls from it. An image records its
applicability, deviations, architectures, capabilities, and expected evidence
in a hardening profile; states its own requirements and maps them to criteria;
decides the controls the baseline leaves to it, with a statement of what
carries each; and is judged by the conformance workflow, which reads evidence
strictly, scores each architecture separately, and answers three questions
apart from each other: is the evidence valid, what is the score, and is the
image release eligible. Releases publish a multi-architecture index without
rebuilding, and a clean runner verifies what was published. The reference web
server does all of it, and publishes its badges to the `badges` branch.

**The next thing to do**, and the only thing blocking the reference image from
being release eligible, is to review its 57 decisions and record the reviewer.
Everything else is genuinely optional sequencing.

**Decisions waiting on a person**, not on work:

- [ ] Record the review of the reference image's 57 decisions
      ([decisions.json](../examples/reference-web-server/decisions.json)).
      Suggested order: the 35 account and identity controls, the 9 the
      deployment configures, the 3 logging controls the image owns, the 5 left
      deliberately open, then the rest.
- [ ] Accept or amend
      [ADR-0008](adr/0008-retrieve-by-locked-location-and-hold-the-pipeline-to-the-standard.md),
      which is proposed while its code is merged and passing.
- [ ] Settle four readings left open in review: AU-10's non-repudiation, the
      four controls that rest on a deployment choosing to mount TLS (SC-8,
      SC-8(1), SC-23, AC-17(2)), SC-10 where the shipped configuration sets no
      keepalive timeout, and IA-7 beside the open FIPS question (SC-13).

**Where to pick it up.** [ADOPTING.md](ADOPTING.md) is the entry point for an
image repository; [EVIDENCE.md](EVIDENCE.md) defines what CI must produce;
[CYBER-PACKAGE.md](CYBER-PACKAGE.md) defines what a person writes. The
decisions above are each a small pull request.
...
- [ ] Review the reference image's 57 decisions in
  ...
  the review; until then it is not release eligible. This is the next thing to
  do; see the status section above.
- [ ] Decide the reference image's Web Server and Application Server SRGs rule
  by rule, in worksheets generated by
  [`scripts/worksheets.py`](../scripts/worksheets.py), and cite them from its
  profile as the basis of each determination. 239 rules, and the worksheet
  suggests which rules of the other SRG share a CCI.
...
- [ ] Decide whether an image's cyber package is published anywhere, or stays
  in its repository. The reference image's is in the repository, and its
  badges are published from a branch.
